...
vi.mock('@fastgpt/service/support/wallet/usage/controller', () => ({
  createPdfParseUsage: vi.fn().mockResolvedValue(undefined)
}));
...
describe('readS3FileContentByBuffer', () => {
  ...
  it('should fallback to system parse for pdf when textinAppId is configured (textin no longer supported)', async () => {
    global.systemEnv = {
    ...
    // textin path was removed; falls back to system parse
    expect(result.rawText).toBe('parsed-pdf-content');
  });
  ...
  it('should reject when custom URL service has url but missing key', async () => {
    global.systemEnv = {
    ...
    ).rejects.toThrow('customParseMissingKey');
  });
  ...
  it('should skip image upload when imageKeyOptions is not provided', async () => {
    ...
    // Without imageKeyOptions, image placeholders remain in the text (no processing)
    expect(result.rawText).toContain('IMAGE_abc123_IMAGE');
  });
});
