/* pg vector crud */
import { DatasetVectorTableName, DBDatasetVectorTableName, DBDatasetValueVectorTableName } from '../constants';
...
export class PgVectorCtrl {
  ...
  databaseEmbRecall = async (props: DatabaseEmbeddingRecallCtrlProps): Promise<DatabaseEmbeddingRecallResponse> => {
    const {
      teamId,
      datasetIds,
      vector,
    ...
    try {
      ...
      const forbidCollectionFilter = forbidCollectionIdList.length > 0
