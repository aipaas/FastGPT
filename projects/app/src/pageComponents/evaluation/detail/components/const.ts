...
// 原有的评测结果映射（保持兼容性）
export const reviewResOptionsMap: Record<EvaluationStatus, string> = {
    [EvaluationStatus.HighQuality]: i18nT('dashboard_evaluation:high_quality'),
    [EvaluationStatus.NeedsImprovement]: i18nT('dashboard_evaluation:needs_improvement')
}

export const reviewResOptions = Object.entries(reviewResOptionsMap).map(([key, value]) => ({
    label: value,
    value: key
})) as { label: EvaluationStatus, value: string }[]
