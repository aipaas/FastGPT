...
import { Box, Button, Flex, IconButton, Divider } from '@chakra-ui/react';
...
const DatasetImportContextProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const modeSteps: Record<ImportDataSourceEnum, { title: string }[]> = {
    ...
    [ImportDataSourceEnum.editDatabase]: [
      {
        title: t('dataset:connect_database')
      },
      {
  ...
  const contextValue = {
    ...
    activeStep,
    tab,
    isEditMode: [ImportDataSourceEnum.editDatabase].includes(source),
    goToNext,
    ...
    return (
      <>
        <Box position="relative">
          <Box position="absolute" alignItems={'center'} top="50%" left="16px" transform="translateY(-50%)">
            <IconButton
              icon={<MyIcon name={'common/backFill'} w={'14px'} />}
              aria-label={''}
              size={'smSquare'}
          ...
          <Flex flex="1" w="calc()" justifyContent="center" mr="70px">
            <FillRowTabs
              py={1}
              value={tab}
  ...
  const renderStep = () => source === ImportDataSourceEnum.editDatabase ? renderUpdateStatusStep() : renderCreateStatusStep()
