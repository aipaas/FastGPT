...
import { DatasetCollectionDataProcessModeEnum, DatasetCollectionTypeEnum, TrainingModeEnum } from '@fastgpt/global/core/dataset/constants';
...
import { ColumnStatusEnum, TableStatusEnum } from './detectChanges';
import { createOneCollection, delCollection } from '@fastgpt/service/core/dataset/collection/controller';
...
function hasColumnForbidInconsistency(existingTable: any, newTable: DBTableProp): boolean {
  ...
  for (const [colName, newCol] of Object.entries(newColumns)) {
    const existingCol = existingColumns.get ? existingColumns.get(colName) : existingColumns[colName];
...
async function handler(
  ...
  try {
          ...
          switch (table.status) {
            case TableStatusEnum.add: {
              ...
              const collection = await MongoDatasetCollection.create([{
                ...
                tableSchema: {
                  ...
                  columns: new Map(Object.entries(table.columns).map(([name, col]) => [
                    name,
                    {
            ...
            case TableStatusEnum.available: {
              ...
              if (existingCollection) {
                ...
                if (existingColNames.size !== newColNames.size ||
                              ...
                              columns: Object.fromEntries(
                                Object.entries(table.columns).map(([name, col]) => [
                                  name,
                                  {
                                    ...
                                    isForeignKey: table.foreignKeys?.some(fk => fk.constrainedColumns.includes(name)) || false,
                                    relatedColumns: [],
                                    metadata: {}
                                  }
                                ])
    ...
    addLog.info(`Successfully processed database changes for dataset ${datasetId}: ${addedTables} added, ${deletedTables} deleted, ${updatedTables} updated`);
