...
import { DatasetCollectionTypeEnum, DatasetCollectionDataProcessModeEnum, TrainingModeEnum } from '@fastgpt/global/core/dataset/constants';
  ...
  const collectionIds: string[] = [];
    ...
    await mongoSessionRun(async (session) => {
      for (const table of tables) {
        ...
        addLog.info(`Created database collection for table ${table.tableName}, training will be processed asynchronously`);
      }
    });

    addLog.info(`Successfully created ${collectionIds.length} database collections for dataset ${datasetId}`);
