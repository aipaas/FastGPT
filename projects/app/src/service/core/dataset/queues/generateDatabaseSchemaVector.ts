...
import {
  insertTableDescriptionVector,
  insertCoulmnDescriptionVector,
  insertTableValueVector,
  deleteDatasetDataVector
} from '@fastgpt/service/common/vectorDB/controller';
import { DBDatasetVectorTableName, DBDatasetValueVectorTableName } from '@fastgpt/service/common/vectorDB/constants';
...
const processDatabaseSchema = async ({
  ...
  if (isRebuild) {
    try {
      ...
      addLog.warn(`[DB Schema] Failed to delete existing indexes for collection ${collectionId}`, error);
    }
  }
  ...
  for (const [columnName, columnInfo] of Object.entries(columns as Record<string, ColumnSchemaType>)) {
    ...
    try {
      ...
      if (columnInfo.examples && Array.isArray(columnInfo.examples)) {
        ...
        const valuePromises = columnInfo.examples.slice(0, 3).map(async (example: string, index: number) => {
        ...
        const valueTokens = valueResults.reduce((sum: number, result: any) => sum + (result.result.tokens || 0), 0);
            ...
            indexes: [
              {
                ...
                dataId: columnDesResult.insertId,
  ...
  if (session && !isRebuild) {
    try {
      ...
      const q = `${tableName}`;
      const a = collection.tableSchema.description ? `${tableName}<sep>${collection.tableSchema.description}` : tableName;
  ...
  addLog.info(`[DB Schema] Processed table ${tableName} with ${columnDesResults.length} columns and ${valueIndexResults.length} value examples`);
